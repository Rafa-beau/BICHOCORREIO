...
var err_pass = 0
@onready var combo_ui = $HudLayer/ComboUI

var current_timer_id: int = 0
@onready var parent = %CardParent
@onready var paw_parent = %PawParent
var vel = 15
...
func _process(delta: float) -> void:
	if timer_active:
		...
		hud.timeleft = max(time_remaining, 0.0)
...
func start_card_timer():
	...
	time_remaining = PlayerManager.time_per_prova
	timer_active = true
...
func step3(i):
	...
	match i:
		...
		3:
			...
			current_paw = Utils.spawn_scene(paw, paw_parent, Vector2(198, -2000))
...
func exec_turn():
	while turn_controler < turn_index:
		...
		turn_controler += 1
		hud.cardleft = turn_index - turn_controler
	end_turn()
# finalizar turno
func end_turn():
	TransitionScene.play_in()
...
func call_card():
	...
	if current_card.ball == false:
		current_paw = Utils.spawn_scene(paw, paw_parent, Vector2(198, -2000))
		current_paw.change_sprite(current_card.water)
