...
func set_text_and_desc():
	match upgrade_index:
		...
		upgrades.JUSTICA:
			...
			if inverted == true:
				upgrade_name += "[/tornado][color=dodger_blue][shake]  INVERTIDA"
				upgrade_desc += "Diminui o [color=green]TEMPO POR PROVA[/color] em [color=green]0.5[/color]"
				return
			if enchanted == true:
				upgrade_name+= "[/tornado][color=medium_orchid][wave]  ENCANTADA"
				upgrade_desc += "[color=green]2 PROVAS ALEATÓRIAS[/color] por [color=green]TURNO[/color] não serão contadas como [color=green]ERROS[/color]"
				return
			upgrade_desc += "Aumenta o [color=green]TEMPO POR PROVA[/color] em [color=green]0.5[/color]"
			pass
		upgrades.ENFORCADO:
